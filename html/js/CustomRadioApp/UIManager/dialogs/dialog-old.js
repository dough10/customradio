...
class InfoDialog {
  constructor() {
    ...
    this.$infoDialog = document.querySelector(selectors.infoDialog);

    const required = [
      this.$infoButton, this.$infoDialog
    ];

    if (required.some(el => !el)) {
      console.error("InfoDialog initialization failed — missing DOM elements.");
      return;
    }

    em.add(this.$infoButton, 'click', this._info.bind(this));
  }
...
class AddStreamDialog {
  constructor() {
    this.$addDialog = document.querySelector(selectors.addDialog);
    this.$addButton = document.querySelector(selectors.add);
    this.$stationSubmitForm = document.querySelector(selectors.stationSubmitForm);
    this.$inputElement = document.querySelector(selectors.stationUrl);

    const required = [
      this.$addDialog, this.$addButton, 
      this.$stationSubmitForm, this.$inputElement
    ];
    ...
    em.add(this.$addButton, 'click', this._openAddDialog.bind(this));
    
    // submit button for add stream dialog form submission
    em.add(this.$stationSubmitForm, 'submit', this._submitStation.bind(this));
    
    // toggle submit button activity on Valid URL input
    em.add(this.$inputElement, 'input', this._toggleButtonActivity.bind(this));
  }
...
class Dialogs {
  constructor() {
    ...
    this._dialogs.forEach(dialog => {
      em.add(dialog, 'click', this._wobbleDialog.bind(this));
    });
  ...
  /**
   * gets all dialog elements
   * 
   * @returns {NodeList<HTMLElement>} all dialog elements
   */
  get _dialogs() {
  ...
  /**
   * gets all dialog close buttons
   * 
   * @returns {NodeList<HTMLElement>} all dialog close buttons
   */
  get _closeButtons() {
